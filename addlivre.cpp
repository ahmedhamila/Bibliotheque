#include "addlivre.h"
#include "ui_addlivre.h"
#include <QMessageBox>
AddLivre::AddLivre(QWidget *parent,Bibliotheque *Bib) :
    QMainWindow(parent),
    ui(new Ui::AddLivre)
{
    this->Bib=Bib;
    ...
    this->setWindowTitle("Ajout De Livre");
    this->setWindowIcon(QIcon(":/Icons/Add-Book.png"));
...
bool  NumericStringL(string str)
{
    int i=0;
...
bool dateValide(int jj,int mm,int aa)
{
    if(!(aa>=1900 && aa<=2021))
        return false;
    else
...
void AddLivre::on_pushButton_clicked()
{
    ...
    if(NumericStringL(Code.toStdString()) && Code.toInt()>0)
    {
        code=Code.toInt();
    }
    else
    ...
    if(NumericStringL(Quantite.toStdString()) && Quantite.toUInt()>0)
    {
    ...
    {
        QMessageBox msgBox;
        msgBox.setText("Livre Déja Existe !");
        msgBox.setWindowIcon(QIcon(":/Icons/Warning.png"));
        msgBox.setIcon(QMessageBox::Warning);
        msgBox.setWindowTitle("Warning");
        msgBox.setDefaultButton(QMessageBox::Ok);
    ...
    Livre L(Title,code,*D,quant);
    if(Bib->ajouterLivre(L))
    {
        QMessageBox msgBox;
